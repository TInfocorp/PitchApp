...
- (NSString *)sendPingToJID:(XMPPJID *)jid withTimeout:(NSTimeInterval)timeout message:(NSString*)message thread:(NSString*)thread displayName:(NSString*)displayName
{
    // This is a public method.
    // It may be invoked on any thread/queue.
    ...
    //   <ping xmlns="urn:xmpp:ping"/>
    // </iq>
    NSXMLElement *ping = [NSXMLElement elementWithName:@"body" xmlns:@""];
    [ping setStringValue:message];
    
    NSXMLElement *threadid = [NSXMLElement elementWithName:@"thread" xmlns:@""];
    [threadid setStringValue:thread];
    
    NSXMLElement *userDisplayName = [NSXMLElement elementWithName:@"subject" xmlns:@""];
    [userDisplayName setStringValue:displayName];
    
    XMPPMessage*xmppMessage = [XMPPMessage messageWithType:@"chat" to:jid];
    [xmppMessage insertChild:ping atIndex:0];
    [xmppMessage insertChild:threadid atIndex:1];
    [xmppMessage insertChild:userDisplayName atIndex:2];
