...
- (void)setValue:(id)value forAttributePath:(NSString *)path
{
    ...
    if ([value isEqual:[NSString stringWithFormat:@"0,0,%@,%@",[[NSUserDefaults standardUserDefaults]objectForKey:@"outerRadius"],[[NSUserDefaults standardUserDefaults]objectForKey:@"outerRadius"]]]) {
       // NSValue *rect =[NSValue valueWithCGRect:CGRectMake(0, 0, 100, 100)];
        NSInteger radius = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults]objectForKey:@"outerRadius"]] integerValue];
         NSValue *rect =[NSValue valueWithCGRect:CGRectMake(0, 0, radius, radius)];
        [self.attributesDictionary setValue:rect forKey:path];
    }
