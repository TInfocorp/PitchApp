...
//#import "SteyelrSingletonClass.h"
...
//static const CGFloat kDefaultCropWidth = 300;
//static const CGFloat kDefaultCropHeight = 200;


 CGFloat kDefaultCropWidth = 300;
 CGFloat kDefaultCropHeight = 200;
...
- (CGRect)cropRect
{
   // kDefaultCropWidth  = self.view.frame.size.width-20;
    if(self.frameView.cropRect.size.width == 0 || self.frameView.cropRect.size.height == 0) {
       ...
       // NSLog(@"%@",[NSValue valueWithCGRect:self.frameView.frame]);
    }
...
{
    NSLog(@"inside viewdidloadad");
    self.addBanner.hidden = NO;
    if (!bannerIsVisible)
    {
//        // If banner isn't part of view hierarchy, add it
//        if (self.addBanner.superview == nil)
//        {
//            [self.view addSubview:self.addBanner];
//        }
...
- (void)viewDidLoad
{
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
        kDefaultCropWidth = 700;
        kDefaultCropHeight = 466;
        
    }
    else{
        int width = self.view.frame.size.width-20;
       // NSLog(@"%@",[NSValue valueWithCGRect:self.frameView.frame]);
        self.frameView.frame = CGRectMake(self.frameView.frame.origin.x, self.frameView.frame.origin.y, width, width*.67);
        kDefaultCropWidth = width;
        kDefaultCropHeight = self.frameView.frame.size.height;
    }
...
- (IBAction)doneAction:(id)sender
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSData *ImageData = UIImageJPEGRepresentation(transform,0.5);
          
            UIImage *img = [UIImage imageWithData: ImageData];
//            [[NSUserDefaults standardUserDefaults] setObject:ImageData forKey:@"imagesaved"];
            [[NSUserDefaults standardUserDefaults]setObject:@"Image" forKey:@"getImage"];
            [[NSUserDefaults standardUserDefaults]synchronize];
            [[self delegate] getImage:nil image:img];
